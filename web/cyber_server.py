...
_NOT_AUTH_REASON = "Authentication required. Set FRIDAY_CYBER_API_KEY or FRIDAY_API_KEY env var."
...
def is_valid_origin(origin):
    ...
    return normalized in _CORS_ORIGINS
...
async def api_command(body: dict, request: Request):
    ...
    if len(command) > _MAX_COMMAND_LENGTH:
        return {"ok": False, "error": f"Command too long (max {_MAX_COMMAND_LENGTH} chars)"}
    if not check_rate("cyber_api_command", rate=0.2, burst=3):
    ...
    try:
        while True:
            ...
            if not command:
                ...
                continue
            if len(command) > _MAX_COMMAND_LENGTH:
                await websocket.send_json({"type": "error", "content": f"Command too long (max {_MAX_COMMAND_LENGTH} chars)"})
                await websocket.send_json({"type": "prompt"})
                continue
            if not check_rate("cyber_ws_command", rate=0.2, burst=3):
...
def main():
    ...
    if not has_custom_key:
        print(f"  Confirm code: ****{API_KEY[-4:]}")
        print(f"  Set FRIDAY_CYBER_API_KEY env var to use your own key.\n")
    else:
        print(f"  API Key: ****{API_KEY[-4:]} (from env var)\n")
